...
-- 2005뇬 6월 17일 ~ 2005년 7월 19일을 포함한 날짜 조회
...
-- LIMIT으로 101번재부터 10개의 데이터 조회
SELECT * FROM customer ORDER BY customer_id ASC LIMIT 100, 10;

-- 데이터 100 개를 건너뛰고 데이터 10개 조회 (오프셋은 데이터를 100개 건너뜀)
...
-- [_]를 사용해 데이터 조회 (이름이 k로 시작하거나 n으로 끝나는 고객)
SELECT * FROM customer WHERE first_name REGEXP 'K[L-N]';

-- [^...]를 사용해 데이터 조회  (k 뒤에 L, M, N을 제외한 문자가 오는 이름을 찾아라)
SELECT * FROM customer WHERE first_name REGEXP 'K[^L-N]';

-- %와 [...]을 사용해 데이터 조회
SELECT * FROM customer WHERE first_name LIKE 'S%' AND first_name REGEXP 'A[L-N]';

-- 와일드카드 조합으로 데이터 조회 (글자 수는 딱 5글자이면서, 이름 중간에 'AL', 'AM', 'AN' 중 하나가 포함되어 있고, 마지막은 숫자 '0'으로 끝나는 고객)
SELECT * FROM customer WHERE first_name LIKE '_____' AND first_name REGEXP 'A[L-N]' AND first_name REGEXP '0$';
...
-- special_feature, rating 열 순서로 데이터를 그룹화
...
-- SELECT 문과 GROUP BY문의 열 이름을 달리할 경우 (그룹바이에 명시한 부분이 그룹되지 않아서 오류가 뜸)
...
-- DISTINCT 문에 COUNT 함수를 사용할 경우 (그룹바이가 안썼다... 카운트만 쓰면 오류가 난다.)
